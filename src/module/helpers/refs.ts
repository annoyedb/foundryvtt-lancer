...
export function limitedUsesIndicator(
  ...
  return `<div class="clipped card limited-card ${nonInteractive}"><span>${game.i18n.localize("lancer.common.descriptor.uses.label").toUpperCase()}</span> ${hexes.join("")}</div>`;
}
...
export function reserveUsesIndicator(path: string, options: HelperOptions): string {
  ...
  return `<div class="clipped card limited-card"><span>${game.i18n.localize("lancer.common.descriptor.uses.label").toUpperCase()}</span> ${hexes.join("")}</div>`;
}
